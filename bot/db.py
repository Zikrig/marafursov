...
DEFAULT_GREETING_TEXT = (
    ...
    "Челлендж «30 дней для заявки» поможет Вам за 30 дней пройти путь от идеи до сформированной заявки на грантовый конкурс или акселератор или стажировку.\n\n"
    ...
    "Обращаем Ваше внимание, что все ваши ответы фиксируются и впоследствии послужат основой для заявки, поэтому будьте внимательны при подготовке ответов.\n"
...
def get_app_settings(db: Session) -> AppSettings:
    ...
    if s:
        changed = False
        # SQLite ALTER TABLE adds new columns as NULL -> ensure defaults are populated.
        if getattr(s, "greeting_text", None) is None:
            s.greeting_text = DEFAULT_GREETING_TEXT
            changed = True
        if getattr(s, "final_text", None) is None:
            s.final_text = DEFAULT_FINAL_TEXT
            changed = True
        if changed:
